...
2. **Never hardcode application-specific URL patterns, credentials, or domain names inside agent logic.** Agent code must derive all test targets from `discovered_endpoints`, `auth_discovered_links`, and `tech_stack` in SharedContext — not from static lists tied to a specific app (e.g. Juice Shop `/api/feedback`, DVWA `/vulnerabilities/sqli/`). Hardcoded patterns cause agents to hallucinate endpoints on other targets, pollute precision metrics, and break generalizability. Violation confirmed in `input_validation_agent.py` `critical_api_patterns` list (fixed 2026-04-24).
...
4. **Never change `_orchestrator_had_plan` logic without running the Juice Shop regression scan.** If set `True` when no `tool_plan` was provided, all agents skip per-agent LLM planning → lose LLM-generated arguments → major recall regression.
5. **`ReportGenerationAgent` must always run**, even when earlier agents fail. Do not add early-exit logic that bypasses it.
6. **Never add `shell=True` to subprocess calls.** All subprocess calls must use list arguments.
7. **`whitelist_domain` must be appended before `validate_target` is called** in `api/routes/scans.py`. Appending after causes 403 for new domains.
8. **When modifying orchestrator phases**, verify job status transitions stay correct in both `orchestrator.py` and `tasks/tasks.py`.
...
### Latest Results — juiceshop_run9 (2026-04-29, OWASP Juice Shop)
...
Missed: 1 SSRF (WSTG-INPV-19, no out-of-band infra) + 4 Expert browser-interaction challenges.
Precision improved 75.9%→94.87% via 5 WSTG code fixes + alias corrections.
Full summary: `multi_agent_system/evaluation/evaluation_juiceshop_summary.json`
