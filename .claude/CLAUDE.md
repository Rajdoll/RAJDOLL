...
### VDP Expansion — BSSN VVIP Program (Planned 2026-03-29)

RAJDOLL is being expanded from Juice Shop-only to real-world government VDP targets (BSSN VVIP Bug Bounty Program). This requires infrastructure generalization without touching any existing tools or metrics.

**Design spec:** `docs/superpowers/specs/2026-03-29-vdp-expansion-design.md`

**Source Code Audit Findings (2026-03-29):**

| Severity | Count | Key Issues |
|----------|-------|-----------|
| Critical | 2 | Hardcoded `admin_token_placeholder`; container_name injection in logs API |
| Medium | 4 | Juice Shop creds in DEFAULT_CREDENTIALS; hardcoded whitelist defaults; fallback URL; KG default entity |
| Low | 2 | No CORS middleware; unpinned openai dependency |

**Files to change (8 total, no new abstractions):**
- `api/schemas/schemas.py` — add `ScanCredentials` model + `credentials`/`whitelist_domain` fields to `CreateScanRequest`
- `api/routes/scans.py` — handle `whitelist_domain` auto-add, store credentials to SharedContext
- `multi_agent_system/utils/session_service.py` — remove `admin@juice-sh.op/admin123`, check SharedContext first
- `multi_agent_system/core/security_guards.py` — `ADMIN_TOKEN` from env var, empty default whitelist
- `multi_agent_system/agents/base_agent.py` — replace fallback URL with `raise ValueError`
- `multi_agent_system/utils/knowledge_graph.py` — accept `target` as constructor param
...
**Juice Shop scan after fix (identical recall):**
```bash
curl -X POST http://localhost:8000/api/scans \
  -H "Content-Type: application/json" \
  -d '{
...
## TODO — VDP Expansion (2026-03-29)

Implementation plan: `docs/superpowers/plans/2026-03-29-vdp-expansion-plan.md` (generated by writing-plans skill)

### Phase 1 — Source Code Audit Fixes (8 files)
- [ ] C1: `security_guards.py:263` — `ADMIN_TOKEN` from env var
- [ ] C2: `api/routes/logs.py:181` — container_name allowlist validation
- [ ] M1: `session_service.py:54` — remove `admin@juice-sh.op/admin123`
- [ ] M2: `security_guards.py:80-98` — empty default whitelist
- [ ] M3: `base_agent.py:964` — raise ValueError instead of fallback URL
- [ ] M4: `knowledge_graph.py:159` — accept target as constructor param
- [ ] L1: `api/main.py` — add CORSMiddleware
- [ ] L2: `requirements.txt` — pin openai version

### Phase 2 — Extended Scan API
- [ ] `api/schemas/schemas.py` — add ScanCredentials model + new fields
- [ ] `api/routes/scans.py` — handle whitelist_domain + credentials injection
- [ ] `session_service.py` — check SharedContext credentials first in auto_login

### Phase 3 — Audit Report & Validation
- [ ] `docs/audit/source_code_audit_report.md` — write thesis audit report
- [ ] Juice Shop regression test: verify ≥ 98% recall with credentials in request
- [ ] BSSN VDP test: verify scan runs end-to-end on real target
