...
def _finding_agent(finding: Dict) -> str:
    return str(finding.get("agent_name") or finding.get("agent") or "")
...
def analyze_scan_findings(findings: List[Dict], *, classifier=None) -> Dict:
    ...
    for cat_id, cat_info in WSTG_TEST_CASES.items():
        ...
        agent_has_findings = any(
            _finding_agent(f) == cat_info["agent"] or _finding_category(f).startswith(cat_id)
            for f in findings
        )
...
def print_coverage_report(
    ...
    *,
    ...
    if source_label:
        print(f"  Source: {source_label}  |  {result['total_findings']} findings  |  mode={mode}")
    else:
        print(f"  Job #{job_id}  |  {result['total_findings']} findings  |  mode={mode}  |  {scan_time_str}")
    print("=" * 78)
    ...
    print(f"    Research target:     >= {target_pct:.1f}%  [{'PASS' if result['coverage_pct'] >= target_pct else 'FAIL'}]")
...
def _extract_findings(payload) -> List[Dict]:
    """Normalize API/file payload variants into a findings list."""
    ...
    if isinstance(payload, dict):
        for key in ("findings", "data", "results"):
            ...
            if isinstance(value, list):
                return value
    raise ValueError("Expected a findings list or an object containing findings/data/results list")


def load_findings_from_file(path: str | Path) -> List[Dict]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    return _extract_findings(payload)


def fetch_findings_from_api(base_url: str, job_id: int, mode: str) -> Tuple[List[Dict], str]:
    import requests

    base = base_url.rstrip("/")
    resp = requests.get(f"{base}/api/scans/{job_id}/findings", params={"mode": mode}, timeout=10)
    resp.raise_for_status()
    findings = _extract_findings(resp.json())

    try:
        job_resp = requests.get(f"{base}/api/scans/{job_id}", timeout=10)
        job_data = job_resp.json()
        scan_time_str = f"Status: {job_data.get('status', '?')}"
    except Exception:
        scan_time_str = ""

    return findings, scan_time_str


def write_json_report(path: str | Path, findings: List[Dict], result: Dict, metrics: Dict, *, mode: str, source: str, target_pct: float) -> None:
    output = {
        "source": source,
        "mode": mode,
        "target_pct": target_pct,
        "target_met": result["coverage_pct"] >= target_pct,
        "result": result,
        "metrics": metrics,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Measure RAJDOLL finding coverage against automatable OWASP Juice Shop challenges.",
    )
    parser.add_argument(
        "job_id",
        nargs="?",
        type=int,
        default=1,
        help="Scan job id to fetch from the API when --input is not used.",
    )
    parser.add_argument(
        "--base-url",
        default="http://localhost:8000",
        help="RAJDOLL API base URL.",
    )
    parser.add_argument(
        "--mode",
        choices=("raw", "reportable", "validated"),
        default="raw",
        help="Finding gate mode to evaluate. Research benchmark should normally use raw or reportable; bug bounty final reports use validated.",
    )
    parser.add_argument(
        "--input",
        help="Read findings from a JSON file instead of fetching the API.",
    )
    parser.add_argument(
        "--output",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        if args.input:
            ...
            findings, scan_time_str = fetch_findings_from_api(args.base_url, args.job_id, args.mode)
            source = f"{args.base_url.rstrip('/')}/api/scans/{args.job_id}/findings?mode={args.mode}"
    except Exception as e:
        print(f"Error loading findings: {e}")
        return 1

    result, metrics = print_coverage_report(
        findings,
        args.job_id,
        scan_time_str,
