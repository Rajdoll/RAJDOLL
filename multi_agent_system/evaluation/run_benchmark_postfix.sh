...
log "=== RAJDOLL v15.0.0 FINAL Re-scan (post XSS+SCA+ATHN-09) -- 5 runs, GT=80, post SSTI+evidence-quality fix ==="
for i in $(seq 1 5); do
  run_scan "juiceshop_postfix_run${i}"
done
log "=== ALL 5 FINAL SCANS DONE ==="

cd /mnt/d/MCP/RAJDOLL
python3 "$EVAL_DIR/compute_metrics.py" --target juiceshop \
  --runs juiceshop_postfix_run1 juiceshop_postfix_run2 juiceshop_postfix_run3 \
         juiceshop_postfix_run4 juiceshop_postfix_run5 2>&1 | tee -a "$LOG"
