...
LOG="$EVAL_DIR/benchmark_9more_$(date +%Y%m%d_%H%M%S).log"
...
print(f'{done}/{total}', running[0] if running else '-')" 2>/dev/null || echo "? -")
...
log "=== RAJDOLL Benchmark -- 9 more runs (run1 = pre-run job #123) ==="
for i in $(seq 2 10); do
  run_scan "juiceshop_benchmark_run${i}"
done
log "=== ALL 9 SCANS DONE ==="
...
python3 "$EVAL_DIR/compute_metrics.py" --target juiceshop \
  --runs juiceshop_benchmark_run1 juiceshop_benchmark_run2 juiceshop_benchmark_run3 \
         ...
         juiceshop_benchmark_run10 2>&1 | tee -a "$LOG"
