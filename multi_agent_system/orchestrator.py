...
from .agents.base_agent import AgentRegistry, AGENT_EXECUTION_TIMEOUT  #  agent runs sequentially so that
	...
	# ReferencesFIX #2: Import timeout
...
class Orchestrator:
	...
	def _run_agent_sync(self, agent_name: str, tool_plan: Optional[Dict[str, Any]] = None):
		...
		if settings.use_framework and settings.use_js_analyzer:
			agent.js_bundle_analyzer = _JSBundleAnalyzer()
	...
	def _run_followup_wave(self) -> None:
		...
		directive, then re-analyze once. No-op unless ADAPTIVE_REPLAN is enabled.
		Additive only - first-pass WSTG coverage is never dropped. One round only.
		"""
		...
		loop = self._ensure_event_loop()
		try:
			probes = loop.run_until_complete(
				asyncio.wait_for(
					summarizer.propose_followup_probes(
						analysis=analysis,
						findings_summary=self.cumulative_summary[-3000:] if self.cumulative_summary else "",
			...
			print(f"[Orchestrator] Follow-up probe proposal failed: {e}")
			return
		if not probes:
			print("[Orchestrator] Follow-up wave: no actionable leads")
			return
		max_probes = getattr(settings, "max_followup_probes", 4)
		ran_any = False
		for probe in probes[:max_probes]:
			...
			try:
				directive = OrchestratorDirective(focus_instructions={agent: focus})
				self._accumulated_directive = merge_directives(self._accumulated_directive, directive)
				print(f"[Orchestrator] Follow-up wave: re-running {agent} "
				      f"(reason: {str(probe.get('reason',''))[:80]})")
				self._run_agent_sync(agent)
				ran_any = True
			except Exception as e:
				print(f"[Orchestrator] Follow-up probe on {agent} failed: {e}")
				continue
		if ran_any:
