...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Post-agent checkpoint buttons
    document.getElementById('cpProceed').addEventListener('click', () => cpAllow());
    document.getElementById('cpAutoAll').addEventListener('click', () => cpAllowAll());
    document.getElementById('cpTellDifferently').addEventListener('click', () => cpOpenTell());

    addLog('[SYSTEM] RAJDOLL initialized. Ready to scan.', 'success');

    // Keyboard shortcuts for HITL panels
    document.addEventListener('keydown', handleHitlKeyboard);
...
function handleHitlKeyboard(e) {
    const prePanel = document.getElementById('pre-agent-panel');
    const postPanel = document.getElementById('checkpointPanel');
    const preVisible = prePanel && prePanel.style.display !== 'none';
    const postVisible = postPanel && postPanel.style.display !== 'none';

    if (!preVisible && !postVisible) return;

    // Enter = Allow (only when textarea not focused)
    if (e.key === 'Enter' && document.activeElement.tagName !== 'TEXTAREA') {
        e.preventDefault();
        if (preVisible) hitlAllow();
        else if (postVisible) cpAllow();
    }

    // Escape = open "Tell differently" textarea
    if (e.key === 'Escape') {
        if (preVisible) {
            const tellSection = document.getElementById('tell-differently-section');
            if (tellSection && tellSection.style.display === 'none') hitlTellDifferently();
            else hitlCancelDirective();
        } else if (postVisible) {
            const tellSection = document.getElementById('cp-tell-section');
            if (tellSection && tellSection.style.display === 'none') cpOpenTell();
            else cpCancelTell();
        }
    }
}
...
async function handleScanSubmit(e) {
    ...
    if (credentials) addLog(`[SYSTEM] Credentials provided for: ${credUsername}`, 'info');
...
async function handleCancelScan() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/scans/${currentJobId}/cancel`, { method: 'POST' });
        if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        const data = await response.json();
...
async function updateStatus() {
    ...
    try {
        ...
        const data = await response.json();
        updateStatusDisplay(data);
        if (data.agents && data.agents.length > 0) updateAgentsDisplay(data.agents);
...
function getAgentIcon(status) {
    const icons = { pending:'◯', queued:'◉', running:'◉', completed:'✓', failed:'✗', skipped:'⊘' };
    return icons[status] || '◯';
}

function formatAgentName(agentType) {
    if (!agentType || typeof agentType !== 'string') return 'Unknown Agent';
    return agentType
        .replace(/([A-Z])/g, ' $1')
        .replace(/_/g, ' ')
        .trim()
...
function _createWebSocket() {
    ...
    if (!isReconnect) addLog(`[SYSTEM] Connecting to real-time log stream (Job ${currentJobId})...`, 'info');
    ...
    try {
        ...
        websocket.onopen = () => {
            if (isReconnect) addLog('[SYSTEM] Reconnected to log stream', 'success');
            else addLog('[SYSTEM] Connected to log stream', 'success');
        ...
        websocket.onmessage = (event) => {
            try {
                ...
                if (suppressServerConnectedMessage && data.type === 'log' &&
                    data.agent === 'SYSTEM' && data.message === 'Connected to log stream') return;
        ...
        websocket.onclose = (event) => {
            ...
            if (event.code === 1000 || isTerminal) { addLog('[SYSTEM] Log stream closed', 'info'); return; }
            if (wsReconnectAttempts < WS_MAX_RECONNECT_ATTEMPTS) {
                wsReconnectAttempts++;
                const delay = WS_BASE_DELAY_MS * Math.pow(2, wsReconnectAttempts - 1);
                addLog(`[SYSTEM] Log stream disconnected. Reconnecting in ${delay/1000}s...`, 'warning');
                wsReconnectTimer = setTimeout(() => _createWebSocket(), delay);
            } else {
                addLog('[SYSTEM] Log stream disconnected. Using status polling only.', 'warning');
            }
        };
    } catch (error) {
...
async function sendIntervention(action) {
    if (!currentJobId) return;
    try {
        addLog(`[HITL] Sending intervention: ${action}`, 'warning');
        const response = await fetch(`${API_BASE}/scans/${currentJobId}/intervene`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action, reason: 'User intervention from dashboard' })
        });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
...
function showPreAgentCheckpoint(d) {
    ...
    // If "allow all" is active, auto-respond immediately
    if (sessionStorage.getItem('hitl_allow_all')) {
    ...
    document.getElementById('tell-differently-section').style.display = 'none';
...
function hitlTellDifferently() {
    document.getElementById('tell-differently-section').style.display = 'block';
    document.getElementById('pre-agent-directive').focus();
}

function hitlCancelDirective() {
    document.getElementById('tell-differently-section').style.display = 'none';
    document.getElementById('pre-agent-directive').value = '';
    document.getElementById('pre-agent-directive-errors').textContent = '';
}

function hitlSendDirective() {
    const text = document.getElementById('pre-agent-directive').value.trim();
    if (!text) { hitlAllow(); return; }
    const errors = validateDirectiveText(text);
    const errEl = document.getElementById('pre-agent-directive-errors');
    if (errors.length > 0) { errEl.textContent = errors.join(' | '); return; }
    errEl.textContent = '';
    _respondPreAgent('proceed', text);
}
...
function showDirectiveHelp() {
    alert(
        'Directive Commands:\n\n' +
        'FOCUS: <path>      — Focus on specific endpoint\n' +
...
function showAgentCheckpoint(data) {
    ...
    el('cpRemaining').textContent = remaining.length > 0 ? `(${remaining.length} remaining)` : '';

    // Reset tell section
    el('cp-tell-section').style.display = 'none';
    if (el('cpUserNotes')) el('cpUserNotes').value = '';
...
function cpAllow() { _respondCheckpoint('proceed', null); }
...
function cpOpenTell() {
    document.getElementById('cp-tell-section').style.display = 'block';
    document.getElementById('cpUserNotes').focus();
}

function cpCancelTell() {
    document.getElementById('cp-tell-section').style.display = 'none';
    document.getElementById('cpUserNotes').value = '';
}

function cpSendTell() {
    const notes = document.getElementById('cpUserNotes').value.trim();
    _respondCheckpoint('proceed', notes || null);
}
...
function toggleHighRiskEdit() {
    ...
    if (edit.style.display === 'none') {
        ...
        try { JSON.parse(edit.value); } catch(e) { alert('Invalid JSON: ' + e.message); return; }
...
async function respondHighRisk(action) {
    ...
    if (document.getElementById('high-risk-args-edit').style.display !== 'none') {
        try {
            body.action = 'edit';
            body.approved_arguments = JSON.parse(document.getElementById('high-risk-args-edit').value);
