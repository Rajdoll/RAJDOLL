...
def test_gate_runs_sqlmap_when_screen_errors(monkeypatch):
    called = {"sqlmap": 0}
    async def fake_screen(*a, **k):
        return {"signal": False, "error": True, "boundary": None, "param": "q",
                "reason": "screen error: timeout"}
    async def fake_sqlmap(*a, **k):
        called["sqlmap"] += 1
        return {"status": "success", "findings": []}
    monkeypatch.setattr(iv, "_sqli_screen", fake_screen)
...
def test_screen_preserves_sibling_get_params(monkeypatch):
    captured = []
    async def fake_send(method, url, **kw):
        captured.append(dict(kw.get("params") or {}))
        return _FakeResp("normal results")
    monkeypatch.setattr(iv, "_sqli_screen_send", fake_send, raising=False)
    asyncio.run(
        iv._sqli_screen("http://t/x?category=1&sort=name", param="sort")
    )
    assert len(captured) > 0
    for params in captured:
        assert params.get("category") == "1"
